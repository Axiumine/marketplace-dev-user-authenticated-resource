...
This project is indexed by GitNexus as **marketplace-dev-user-authenticated-resource**.

> Index stale? Run `node .gitnexus/run.cjs analyze --index-only` from the project root — it auto-selects an available runner. No `.gitnexus/run.cjs` yet? Bootstrap with `npx`, `bunx`, or `pnpm dlx` — e.g. `bunx gitnexus@latest analyze` (npm 11 npx crash; #1939).

> This block does **not** self-heal: this repo's `.gitnexusrc` now sets `{"analyze": {"skipContextFiles": true}}`, so `gitnexus analyze` never rewrites it — the flag suppresses only this block, the index is still built. Every fact above is hand-maintained; correct it by hand when it drifts.
